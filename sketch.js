...
function setup() {
  ...
  pakman.scale = 0.05
  redG = createSprite(566, 321, 20, 20)
  redG.addImage(redgImage)
  redG.scale = 0.1
  blueG = createSprite(566, 311, 20, 20)
  blueG.addImage(bluegImage)
  blueG.scale = 0.1
  purpleG = createSprite(600, 311, 20, 20)
...
function draw() {
  ...
  if(gameState==="newLife"){
   for(var i = 0;i<path1.length;i++){
     if(redG.x===path1[i][0]&&redG.y===path1[i][1]){
      redG.x=path1[i+1][0];
      redG.y=path1[i+1][1]
     }
   }
...
//    if(blueG.isTouching(walls[i])){
 ...
 function ghostRand(ghost){
  ...
  function createPath1(){
    ...
    for(var i = 225;i<491;i+=5){
      var pos = [664,i]
      path1.push(pos)
    }
    for(var i = 491;i<572;i+=5){
      var pos = [895,i]
      path1.push(pos)
    }
    for(var i = 895;i>695;i-=5){
      var pos = [i,572]
      path1.push(pos)
    }
    for(var i = 695;i>663;i-=5){
      var pos = [i,572]
      path1.push(pos)
    }
    for(var i = 572;i>490;i-=5){
      var pos = [663,i]
      path1.push(pos)
    }
